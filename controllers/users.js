...
module.exports.createUser = (req, res) => {
  ...
  User.create([{ name, about, avatar }], { runValidators: true }).then((user) => { res.send({ name: user.name, about: user.about, avatar: user.avatar }); }).catch((err) => res.status(400).send({ message: `Произошла ошибка${err}` }));
};
